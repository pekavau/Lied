...
- **Sort & filter.** List endpoints accept `?sort=<field>:<dir>` (e.g. `sort=created_at:desc`) and `?filter[<field>]=<value>` (e.g. `filter[status]=active`); multiple filters AND together. Each endpoint declares an **allowlist** of sortable/filterable fields with a default sort (arrangements default `title:asc`); an unknown field or direction → `400` Problem Details. The allowlist keeps this from becoming arbitrary-column SQL surface. The bracket syntax namespaces filters away from control params (`sort`, `limit`, `offset`, `q`) so they never collide. **Filter keys are percent-decoded before the `filter[…]` prefix is matched** — a browser encodes the brackets when it submits a GET form, and a filter that fails to parse must never be silently dropped (that reads as "nothing was excluded" while returning the unfiltered list).

  **`?q=` on arrangements is full-text + fuzzy** (issue #34, replacing the phase-1 ILIKE without changing the request shape): Postgres FTS over a trigger-maintained `tsvector` spanning title, composer, arranger, tag names and instrumentation, plus `pg_trgm` matching on title and composer so a misspelling still finds the piece. The text is indexed and queried through `simple` + `unaccent`, so search is accent-insensitive in both directions and no stemmer is imposed on a deliberately multilingual catalogue. `websearch_to_tsquery` parses the user's text, which gives quoted phrases and `-exclusion` for free and never raises on punctuation. Fuzzy recall depends on `pg_trgm.similarity_threshold` (default 0.3), because only the `%` operator can use the trigram index — a test pins the shipped behaviour.

  Arrangements additionally accept the faceted filters `difficultyMin`, `difficultyMax`, `durationMinSeconds`, `durationMaxSeconds`, `tag` (comma-separated ids; an arrangement must carry **all** of them) and `instrumentId`, plus a `relevance` sort. Ranges are separate min/max fields rather than a `3..6` value syntax, so each is its own allowlisted name and its own OpenAPI parameter. Bare `sort=relevance` means best-first; without a `q` it falls back to the default sort rather than erroring, so a UI that keeps the sort while the user clears the box does not 400 at them.
...
- **Unique constraints** (all are partial indexes `WHERE deleted_at IS NULL` where the entity has soft-delete, so the constraint applies to live rows only):
  ...
  | `PartAssignment` | `(collection_item_id, voice_id, user_id)` — a part is played by as many musicians as the section has; the same person twice on one part is the mistake |
  ...
  **Several musicians may hold one part** (issue #53): a second violin part is read by every desk of the section, so assigning adds a player rather than replacing one. No `role` discriminator — nobody has needed to distinguish principal from cover yet. One person holding *two* voices of the same piece is permitted (sections get juggled by who turns up) and reported as a conflict, since nobody plays two parts at once.
...
- **Distribution (UC-12) and coverage (UC-13).** No separate distribution table; `PartAssignment.notified_at` and `PartAssignment.acknowledged_at` (both nullable timestamps) carry the state. Coverage check: a part is **covered** once at least one musician holds it (the model has no desk counts, so "enough players" is not a question it can answer); acknowledgement is reported as a **ratio of players**, not a per-part flag, because a section where two of three have replied is neither rehearsed nor unrehearsed. Coverage only ever reports — no operation refuses because a part is unassigned.
